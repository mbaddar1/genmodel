...
def posterior(N: int, d: int) -> np.ndarray:
    ...
    #     mean=-np.ones(d) * 5.0, cov=0.1 * np.eye(d), size=N
    # )
...
rv = tensap.RandomVector(
    # [tensap.UniformRandomVariable(-10, 10)] * d
    [tensap.NormalRandomVariable()] * d
    + [tensap.UniformRandomVariable(0.0, 1.0)]
)
limits = (-15.0, 15.0)
...
X_t = samples_0 * (1.0 - t[:, None]) + samples_1 * t[:, None]  # (N, d)
# X_train = np.column_stack((X_t, t[:, None]))  # (N, d+1)
# Y_train = samples_1 - samples_0  # (N, d)
X_train = np.column_stack((samples_0, t[:, None]))  # (N, d+1)
Y_train = X_t
...
outputs = f(np.column_stack([z0, np.ones((N_test, 1))]))
plt.figure(figsize=(4, 4))
plt.xlim(*limits)
plt.ylim(*limits)
